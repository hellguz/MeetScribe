# Remove Redis & Celery Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace Redis + Celery with a `ThreadPoolExecutor` + `APScheduler` running inside the FastAPI process, collapsing 3 backend services to 1.

**Architecture:** All background work (transcription, summarization, translation, section generation) is submitted to a `ThreadPoolExecutor` via `executor.submit()`. Periodic tasks (cleanup every 15 min, nightly backup) run on an `APScheduler` background thread. A module-level `_executor` variable in `tasks.py` is set at startup so `cleanup_stuck_meetings` can re-queue tasks without a circular import.

**Tech Stack:** Python 3.12, FastAPI, SQLModel, APScheduler 3.x, `concurrent.futures.ThreadPoolExecutor`

**Spec:** `docs/superpowers/specs/2026-03-24-remove-redis-celery-design.md`
...
|---|---|---|
| `backend/app/tasks.py` | **Create** (from `worker.py`) | All background task functions, scheduler functions, no Celery |
| `backend/app/worker.py` | **Delete** after tasks.py is verified | Old Celery module |
| `backend/app/main.py` | **Modify** | Add executor + scheduler, fix imports, replace `.delay()` |
| `backend/app/config.py` | **Modify** | Remove Celery settings, add `worker_threads` |
| `backend/requirements.txt` | **Modify** | Swap Celery/Redis for APScheduler |
| `docker-compose.yml` | **Modify** | Remove redis + celeryworker services |
...
- [ ] **Step 1: Remove Celery/Redis settings from config.py**

In `backend/app/config.py`, remove the two Celery fields and add `worker_threads`. The file should look like this after:
...
In `backend/requirements.txt`, remove the `# Celery & Redis` block and add apscheduler:
...
```bash
python -c "from app.config import settings; print('worker_threads:', settings.worker_threads)"
```
Expected output: `worker_threads: 4`

- [ ] **Step 4: Commit**

```bash
git add backend/app/config.py backend/requirements.txt
git commit -m "refactor: remove Celery/Redis config, add worker_threads and apscheduler"
```

---

## Task 2: Create `tasks.py`

This is the main transformation. We create `tasks.py` as a clean rewrite of `worker.py` with all Celery machinery stripped out.

**Files:**
- Create: `backend/app/tasks.py`

- [ ] **Step 1: Create `backend/app/tasks.py` with the full content below**

Key changes from `worker.py`:
- No `celery` imports, no `@celery_app.task` decorators
- Module-level `_executor = None` + `set_executor()` so cleanup can re-queue tasks
- `process_transcription_and_summary`, `generate_summary_only`, `translate_meeting_sections` get plain retry loops
- `generate_section_content` retry loop preserves error-state-on-final-failure
- `cleanup_stuck_meetings` uses `_executor.submit()` instead of `.delay()`
- Logger renamed from `"celery_worker"` to `"meetscribe_tasks"`
- `"Celery Worker:"` removed from log strings
    ...
    with Session(engine) as db:
        ...
        for mtg in stuck_meetings:
            ...
            if not unprocessed_chunks:
                LOGGER.info(f"Janitor: Meeting {mtg.id} has no unprocessed chunks but isn't done. Re-triggering finalization.")
            ...
            for chunk in unprocessed_chunks:
                ...
                if chunk_path.exists():
                    ...
                    LOGGER.error(f"Janitor: Chunk path {chunk.path} does not exist. Cannot re-queue.")
        ...
        try:
            ...
            with Session(engine) as db:
                ...
                for section in sections:
                    try:
                        if section.title:
                            section.title = translate_text(section.title, target_language, meeting.context)
                        if section.content:
                            section.content = translate_text(section.content, target_language, meeting.context)
                        section.is_generating = False
                        section.updated_at = dt.datetime.utcnow()
                        db.add(section)
                    except Exception as e:
                        LOGGER.error(f"Failed to translate section {section.id} for meeting {meeting_id}: {e}")
...
```bash
cd backend
python -c "from app.tasks import get_db_engine, backup_database, cleanup_stuck_meetings; print('tasks.py imports OK')"
```
Expected: `tasks.py imports OK`

If it fails, check for any missed imports or syntax errors.

- [ ] **Step 3: Commit**

```bash
git add backend/app/tasks.py
git commit -m "feat: add tasks.py — plain-function replacement for Celery worker"
```
...
Replace Celery dispatching with `executor.submit()`, add lifespan with scheduler, fix all imports.
...
- [ ] **Step 2: Add the lifespan context manager and wire up executor + scheduler**
...
- [ ] **Step 4: Replace `.delay()` call in `upload_chunk`**
...
- [ ] **Step 5: Replace `.delay()` call in `get_meeting`**
...
- [ ] **Step 6: Replace both `generate_section_content` call sites in `create_section` and `regenerate_section_content`**

In `create_section` (around line 771-772), replace:
```python
...
In `regenerate_section_content` (around line 907-908), replace:
```python
from .worker import generate_section_content
...
- [ ] **Step 7: Replace `.delay()` call in `translate_meeting`**
...
```
And no import errors or tracebacks. Kill with Ctrl+C.

- [ ] **Step 9: Commit**

```bash
git add backend/app/main.py
git commit -m "feat: replace Celery dispatch with ThreadPoolExecutor + APScheduler in main.py"
```
...
- Removed `celeryworker` service
- Removed `redis-data` volume
- Moved `whisper-cache` volume mount to `backend`
- Added `stop_grace_period: 1m` to `backend`
- Removed `depends_on: redis` from `backend`

- [ ] **Step 2: Commit**

```bash
git add docker-compose.yml
git commit -m "feat: simplify docker-compose — remove redis and celeryworker services"
```

---

## Task 5: Delete `worker.py` and clean up

**Files:**
- Delete: `backend/app/worker.py`

- [ ] **Step 1: Confirm nothing imports worker.py anymore**

```bash
grep -r "from .worker" backend/app/
grep -r "from app.worker" backend/app/
grep -r "import worker" backend/app/
```

Expected: no output. If anything is found, fix those imports before deleting.

- [ ] **Step 2: Delete worker.py**

```bash
git rm backend/app/worker.py
```

- [ ] **Step 3: Commit**

```bash
git commit -m "chore: delete worker.py — superseded by tasks.py"
```
...
Verify `apscheduler` is installed and `celery`/`redis` are gone:
```bash
pip show apscheduler
pip show celery  # should say "not found"
```

- [ ] **Step 2: Start the server and watch startup logs**
...
No `celery`, `redis`, or `broker` mentions.

- [ ] **Step 3: Verify API responds**

```bash
curl http://localhost:8000/api/meetings/sync -X POST \
...
- [ ] **Step 4: Verify a meeting can be created**
...
Expected: JSON with a meeting UUID and status fields. No 500 error.

- [ ] **Step 5: Final commit**

```bash
git add -A
git commit -m "chore: smoke test passed — Redis/Celery removal complete"
```
