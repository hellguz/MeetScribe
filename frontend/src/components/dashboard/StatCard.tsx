...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, color }) => {
	...
	return (
		<div
			...
			<div style={{ fontSize: '24px', backgroundColor: 'rgba(255,255,255,0.1)', padding: '12px', borderRadius: '8px' }}>{icon}</div>
			<div>
				<p style={{ margin: 0, color: color.text, fontSize: '14px', fontWeight: '500' }}>{title}</p>
				<p style={{ margin: '4px 0 0', color: color.text, fontSize: '28px', fontWeight: 'bold' }}>{displayValue}</p>
			</div>
		</div>
	)
}
