...
from .models import (
    ...
    MeetingSection,
    MeetingSectionCreate,
    MeetingSectionUpdate,
    MeetingSectionReorder,
    MeetingSectionResponse,
)
from .worker import process_transcription_and_summary, generate_summary_only, generate_section_content
...
@app.get("/api/meetings/{mid}/sections", response_model=list[MeetingSectionResponse])
def get_meeting_sections(mid: uuid.UUID):
    """Get all sections for a meeting."""
    with Session(engine) as db:
        sections = db.exec(
...
@app.post("/api/meetings/{mid}/sections", response_model=MeetingSectionResponse, status_code=201)
def create_meeting_section(mid: uuid.UUID, section_data: MeetingSectionCreate):
    """Create a new custom section for a meeting."""
    with Session(engine) as db:
        ...
        section = MeetingSection(
            ...
            template_type=section_data.template_type,
            content=None  # Will be generated by AI
        )
        ...
        # Only auto-generate content for new template sections (not when parsing existing summaries)
        # We can tell it's a new template section if the template_type is not 'custom' and 
        # the section_key matches the template_type (indicating it's a fresh template selection)
        if (section_data.template_type and 
            section_data.template_type != 'custom' and 
            section_data.section_key == section_data.template_type):
            # Check if meeting has transcript available
            if meeting.done and meeting.transcript_text:
                generate_section_content.delay(str(section.id))
        
        return section


@app.put("/api/sections/{section_id}", response_model=MeetingSectionResponse)
def update_meeting_section(section_id: int, update_data: MeetingSectionUpdate):
    ...
    with Session(engine) as db:
        ...
        if update_data.is_enabled is not None:
            section.is_enabled = update_data.is_enabled
...
@app.post("/api/meetings/{mid}/sections/reorder")
def reorder_meeting_sections(mid: uuid.UUID, reorder_data: MeetingSectionReorder):
    """Reorder sections for a meeting."""
    with Session(engine) as db:
        # Get all sections for the meeting
        sections = {
            section.id: section for section in db.exec(
                select(MeetingSection).where(MeetingSection.meeting_id == mid)
            ).all()
        }
        
        # Update positions based on the ordered list
        for i, section_id in enumerate(reorder_data.section_ids):
            if section_id in sections:
                sections[section_id].position = i
                sections[section_id].updated_at = dt.datetime.utcnow()
...
def delete_meeting_section(section_id: int):
    ...
    with Session(engine) as db:
        ...
        if section.section_type == "default":
            raise HTTPException(status_code=400, detail="Cannot delete default sections")
...
def generate_section_content_endpoint(section_id: int):
    ...
    with Session(engine) as db:
        ...
        if not meeting.done or not meeting.transcript_text:
            raise HTTPException(status_code=400, detail="Meeting transcript not available")
        
        # Queue the content generation task
        generate_section_content.delay(str(section_id))
        
        return {"status": "generation_queued"}
